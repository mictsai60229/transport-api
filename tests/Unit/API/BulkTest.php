...
class BulkTest extends TestCase{
     ...
     * @depends testIndicesCreateTest
     */
    public function testBulkStartTest(string $create_index): void{

        $index = self::$index;

        $response = $this->postJson("/v1/{$index}/bulk/start", []);
        $response
        ->assertStatus(200)
        ->assertJson([
            "metadata" => [
     ...
     * @depends testIndicesCreateTest
     */
    public function testBulkEndTest(string $create_index): void{

        $index = self::$index;

        $response = $this->postJson("/v1/{$index}/bulk/end", []);
        $response
        ->assertStatus(200)
        ->assertJson([
            "metadata" => [
